...
python send_batch_test_data.py $S3_BUCKET/$PATH_ANNOTATION_PREPROCESSED $PATH_ANNOTATION_DASHBOARD/current-model $CURRENT_MODEL_API_PATH

# Predict with next/retrained model to send data to dashboard
python send_batch_test_data.py $S3_BUCKET/$PATH_ANNOTATION_PREPROCESSED $PATH_ANNOTATION_DASHBOARD/next-model $NEXT_MODEL_API_PATH